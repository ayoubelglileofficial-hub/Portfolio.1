...
import { EducationCreateSchema } from '@/types/education';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const educations = await Education.find(filter).sort({ order_index: 1 }).lean();
...
export async function POST(request: NextRequest) {
  try {
    ...
    await connectDB();
    const education = await Education.create(parsed.data);
    return NextResponse.json(education, { status: 201 });
  } catch (error) {
    return handleMongoError(error, 'Failed to create education');
  }
}
