...
interface SkillsVisibilityToggleProps {
    initialValue?: boolean | string;
    onToggle?: (isVisible: boolean) => void;
    className?: string;
}

export default function SkillsVisibilityToggle({
    initialValue = true,
    onToggle,
    className = "",
}: SkillsVisibilityToggleProps) {
    ...
    const handleToggle = async () => {
        ...
        try {
            const res = await fetch("/api/profile", {
                ...
                body: JSON.stringify({ show_skills: newValue }),
            });

            if (!res.ok) throw new Error("Failed to update skills visibility");
