...
interface ProjectsVisibilityToggleProps {
    initialValue?: boolean | string;
    onToggle?: (isVisible: boolean) => void;
    className?: string;
}

export default function ProjectsVisibilityToggle({
    initialValue = true,
    onToggle,
    className = "",
}: ProjectsVisibilityToggleProps) {
    ...
    const [showProjects, setShowProjects] = useState(normalizedInitialValue);
    const [isLoading, setIsLoading] = useState(false);

    const handleToggle = async () => {
        const newValue = !showProjects;
        ...
        try {
            const res = await fetch("/api/profile", {
                ...
                body: JSON.stringify({ show_projects: newValue }),
            });

            if (!res.ok) throw new Error("Failed to update projects visibility");

            onToggle?.(newValue);
            router.refresh();
        } catch (error) {
            console.error("Error updating projects visibility:", error);
            setShowProjects(!newValue);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        ...
        `}
            ...
            <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
                {showProjects ? "Projects Visible" : "Projects Hidden"}
            </span>
