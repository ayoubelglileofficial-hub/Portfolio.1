...
interface CertificationVisibilityToggleProps {
    initialValue?: boolean | string;
    onToggle?: (isVisible: boolean) => void;
    className?: string;
}

export default function CertificationVisibilityToggle({
    initialValue = true,
    onToggle,
    className = "",
}: CertificationVisibilityToggleProps) {
    ...
    const [showCertification, setShowCertification] = useState(normalizedInitialValue);
    const [isLoading, setIsLoading] = useState(false);

    const handleToggle = async () => {
        const newValue = !showCertification;
        setShowCertification(newValue);
        ...
        try {
            const res = await fetch("/api/profile", {
                ...
                body: JSON.stringify({ show_certification: newValue }),
            });

            if (!res.ok) throw new Error("Failed to update certification visibility");
    ...
    return (
        ...
        `}
            ...
            <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
                {showCertification ? "Certification Visible" : "Certification Hidden"}
            </span>
