...
interface CertificationAdminFormProps {
  item?: (CertificationFormData & { _id?: string }) | null;
  onClose: () => void;
  onSave: () => void;
}
...
export default function CertificationAdminForm({ item, onClose, onSave }: CertificationAdminFormProps) {
  const [form, setForm] = useState<CertificationFormData>(defaultForm);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = isEditing ? `/api/certification/${item!._id}` : '/api/certification';
    ...
    >
      ...
      >
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-zinc-700">
          <div>
            <h2 className="text-lg font-bold">{isEditing ? 'Edit Certification' : 'Add Certification'}</h2>
            <p className="text-sm text-muted-foreground">
              {isEditing ? 'Update certification details' : 'Add a new certification to your portfolio'}
            </p>
          </div>
          <Button
            variant="ghost"
