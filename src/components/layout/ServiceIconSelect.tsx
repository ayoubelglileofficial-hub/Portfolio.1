...
export function ServiceIconSelect({ value, onChange, placeholder = "Search a service icon..." }: ServiceIconSelectProps) {
    const iconKeys = useMemo(() => Object.keys(serviceIconMap), []);
    ...
    const {
        ...
        handleKeyDown,
        resetSearch,
    } = useSearchableDropdown(iconKeys);
    ...
    const selected = value ? getServiceIcon(value) : null;
        ...
        <div ref={containerRef} className="relative w-full">
            <div className="flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500">
                ...
                <input
                    ...
                    onKeyDown={(e) => handleKeyDown(e, selectIcon)}
            ...
            {open && filtered.length > 0 && (
                <ul className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-gray-200 bg-white shadow-lg">
                    {filtered.map((key, i) => {
                        const { icon: Icon, color } = getServiceIcon(key);
                        return (
                            <li
                                key={key}
                                onMouseDown={() => selectIcon(key)}
